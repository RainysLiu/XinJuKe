...
class ExportPlugin(BaseAdminPlugin):
    ...
    def _get_objects(self, context):
        ...
        return [
            dict(
                [
                    (force_text(headers[i].text), self._format_value(o))
    ...
    def _get_datas(self, context):
        ...
        new_rows.insert(
            0, [force_text(c.text) for c in context["result_headers"].cells if c.export]
        )
        return new_rows
    ...
    def get_xlsx_export(self, context):
        ...
        sheet = book.add_worksheet(u"%s %s" % (_(u"Sheet"), force_text(model_name)))
        styles = {
