...
class UEditorWidget(forms.Textarea):
    def __init__(self, attrs=None):
        ...
        self.upload_settings.update(
            {"imagePathFormat": imagePath, "filePathFormat": filePath}
        )
        # 保存
        ...
        if toolbars == "full":
            ...
            isinstance(toolbars, string_types)
            and toolbars in USettings.TOOLBARS_SETTINGS
        ):
    ...
    def recalc_path(self, model_inst):
        ...
        try:
            ...
            if "videoPathFormat" in self._upload_settings:
                uSettings["videoPathFormat"] = (
                    calc_path(self._upload_settings["videoPathFormat"], model_inst),
                )
